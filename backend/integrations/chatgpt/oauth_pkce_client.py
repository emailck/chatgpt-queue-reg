...
class OAuthPkceClient:
    ...
    def create_account(self, name: str, birthdate: str) -> None:
        ...
        if self._device_id:
            sentinel_token = self._fetch_sentinel_token(self._device_id, "oauth_create_account")
            if sentinel_token:
                headers["openai-sentinel-token"] = sentinel_token
                try:
                    import json
                    st = json.loads(sentinel_token)
                    so_st = st.copy()
                    if "p" in so_st:
                        so_st["so"] = so_st.pop("p")
                    headers["openai-sentinel-so-token"] = json.dumps(so_st, separators=(",", ":"))
                except Exception:
                    pass
