...
def describe_request_fingerprint(user_agent=None, sec_ch_ua=None, impersonate=None):
    """格式化请求层 fingerprint，便于统一打印 UA 相关日志。"""
...
def build_browser_headers(
    *,
    ...
    """构造更接近真实浏览器的请求头。Firefox 不发 sec-ch-ua 系列。"""
    is_firefox = "Firefox" in (user_agent or "")
    chrome_full = chrome_full_version or ("" if is_firefox else extract_chrome_full_version(user_agent))
    full_version_list = "" if is_firefox else build_sec_ch_ua_full_version_list(sec_ch_ua, chrome_full)
    ...
    if chrome_full:
        ...
        headers["sec-ch-ua-platform-version"] = sec_ch_ua_platform_version or '"15.0.0"'
