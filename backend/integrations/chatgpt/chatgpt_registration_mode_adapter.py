...
from backend.core.legacy_shim import Account, AccountStatus
...
class BaseChatGPTRegistrationModeAdapter(ABC):
    ...
    def _build_account_extra(self, result) -> dict:
        extra = dict(getattr(result, "metadata", None) or {})
        extra.update({
...
class RefreshTokenChatGPTRegistrationAdapter(BaseChatGPTRegistrationModeAdapter):
    ...
    def _create_engine(self, context: ChatGPTRegistrationContext):
        from backend.integrations.chatgpt.refresh_token_registration_engine import RefreshTokenRegistrationEngine
...
class AccessTokenOnlyChatGPTRegistrationAdapter(BaseChatGPTRegistrationModeAdapter):
    ...
    def _create_engine(self, context: ChatGPTRegistrationContext):
        from backend.integrations.chatgpt.access_token_only_registration_engine import AccessTokenOnlyRegistrationEngine
