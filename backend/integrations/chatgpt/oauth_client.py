...
from .fingerprint import (
    BrowserFingerprint,
    impersonate_from_user_agent,
    is_firefox_user_agent,
    normalize_impersonate,
    random_fingerprint,
    sec_ch_ua_from_user_agent,
)
from .backend_headers import compute_passkey_capabilities
from .backend_headers import extract_oai_client_versions_from_homepage_html
...
class OAuthClient:
    ...
    def __init__(self, config, proxy=None, verbose=True, browser_mode="protocol"):
        ...
        # 创建 session（会被 _ensure_oauth_fingerprint 重建）
        self.session = curl_requests.Session(impersonate=self.impersonate or "chrome136")
        if self.proxy:
    ...
    def adopt_browser_context(
        ...
        *,
        ...
        is_ff = "Firefox" in (user_agent or "")
        if sec_ch_ua and not is_ff:
        ...
        if device_id:
            ...
            if not self.oai_session_id:
                self.oai_session_id = str(uuid.uuid4())
        session_version = str(getattr(self.session, "oai_client_version", "") or "").strip()
        session_build = str(getattr(self.session, "oai_client_build_number", "") or "").strip()
        if session_version:
            self.oai_client_version = session_version
        if session_build:
            self.oai_client_build_number = session_build
        ...
        elif user_agent and not self.impersonate:
            self.impersonate = impersonate_from_user_agent(user_agent)
        elif self.impersonate:
            self.impersonate = normalize_impersonate(self.impersonate)
    ...
    @staticmethod
    def _random_browser_fingerprint():
        fingerprint = random_fingerprint()
        return fingerprint.user_agent, fingerprint.sec_ch_ua, fingerprint.impersonate

    def _ensure_oauth_fingerprint(self, user_agent, sec_ch_ua, impersonate):
        if user_agent and (sec_ch_ua or is_firefox_user_agent(user_agent)) and impersonate:
            impersonate = normalize_impersonate(impersonate)
            self.impersonate = impersonate
            return user_agent, sec_ch_ua, impersonate

        fingerprint = random_fingerprint()
        provided_user_agent = str(user_agent or "").strip()
        provided_sec_ch_ua = str(sec_ch_ua or "").strip()
        user_agent = user_agent or fingerprint.user_agent
        sec_ch_ua = (
            sec_ch_ua
            or (
                sec_ch_ua_from_user_agent(provided_user_agent, default="")
                if provided_user_agent
                else ""
            )
            or fingerprint.sec_ch_ua
        )
        impersonate = (
            impersonate
            or (
                impersonate_from_user_agent(provided_user_agent, default="")
                if provided_user_agent
                else ""
            )
            or fingerprint.impersonate
        )
        self.ua = str(user_agent or "").strip()
        self.sec_ch_ua = str(sec_ch_ua or "").strip()
        self.impersonate = normalize_impersonate(impersonate)
        self._fingerprint = fingerprint
        self.is_firefox = fingerprint.is_firefox
        self.viewport_width = fingerprint.viewport_width
        self.viewport_height = fingerprint.viewport_height

        # 重建 session 以使用正确的 TLS 指纹
        old_cookies = {}
        try:
            for cookie in self.session.cookies:
                ...
                if name:
                    old_cookies[(name, domain)] = cookie
        except Exception:
            pass

        self.session = curl_requests.Session(
            impersonate=self.impersonate or "chrome136",
            proxy=self.proxy,
        )
        if self.proxy:
            self.session.proxies = build_requests_proxy_config(self.proxy)

        # 恢复之前的 cookies（oai-did, oai-client-auth-session 等）
        if old_cookies:
            try:
                for (name, domain), cookie in old_cookies.items():
                    try:
        ...
        try:
            if not provided_user_agent and not provided_sec_ch_ua:
                header_updates = fingerprint.base_headers()
            elif fingerprint.is_firefox:
                header_updates = {
        ...
        self._log(
            f"OAuth 指纹: type={fingerprint.browser_type} "
            f"ua={user_agent[-30:]} impersonate={impersonate}"
        )
        return user_agent, sec_ch_ua, impersonate
    ...
    def _headers(
        ...
        *,
        ...
        try:
            ...
            accept_language = None
        try:
            platform_version = self.session.headers.get("sec-ch-ua-platform-version")
        except Exception:
        ...
        return build_browser_headers(
            ...
            platform=self._fingerprint.platform if hasattr(self, '_fingerprint') and self._fingerprint else "Windows",
            accept=accept,
    ...
    def _session_request(self, method, url, **kwargs):
        self.impersonate = normalize_impersonate(kwargs.get("impersonate") or self.impersonate or "chrome136")
            ...
            try:
                ...
                self._log(
                    ...
                    f"{describe_request_fingerprint(
                        ...
                        impersonate=kwargs.get('impersonate') or self.impersonate,
                    )}"
                )
    ...
    def _follow_flow_state(
        ...
        for hop in range(max_hops):
            try:
                ...
                r = self._session_get(current_url, **kwargs)
                last_url = str(r.url)
    ...
    def _bootstrap_oauth_session(
        ...
        try:
            ...
            self._browser_pause()
            r = self._session_get(authorize_url, **kwargs)
        ...
        try:
            ...
            self._browser_pause()
            r2 = self._session_get(oauth2_url, **kwargs)
        ...
        *,
        ...
        for _sentinel_attempt in range(2):
            ...
            sentinel_token = build_sentinel_token(
                ...
                logger=lambda msg: self._log(f"authorize_continue: {msg}"),
            )
            if sentinel_token:
        ...
        try:
            ...
            self._browser_pause()
            r = self._session_post(request_url, **kwargs)
        ...
        *,
        ...
        if sentinel_pwd:
            ...
            sentinel_pwd = build_sentinel_token(
                ...
                platform=self._fingerprint.platform if hasattr(self, '_fingerprint') and self._fingerprint else "Windows",
                is_firefox=self.is_firefox,
                logger=lambda msg: self._log(f"password_verify: {msg}"),
            )
            if sentinel_pwd:
        ...
        *,
        ...
        if sentinel_token:
            ...
            sentinel_token = build_sentinel_token(
                ...
                platform=self._fingerprint.platform if hasattr(self, '_fingerprint') and self._fingerprint else "Windows",
                is_firefox=self.is_firefox,
                logger=lambda msg: self._log(f"username_password_create: {msg}"),
            )
            if sentinel_token:
    ...
    def _send_signup_email_otp(
        ...
        *,
        ...
        try:
            ...
            self._browser_pause()
            r = self._session_get(request_url, **kwargs)
            ...
            r_verify = self._session_get(verify_url, **verify_kwargs)
    ...
    def _submit_about_you_create_account(
        ...
        *,
        ...
        def _build_create_headers(sentinel_token: str = "", so_token: str = ""):
            extra_headers = {}
            if sentinel_token:
                extra_headers["openai-sentinel-token"] = sentinel_token
            if so_token:
                extra_headers["openai-sentinel-so-token"] = so_token
        ...
        def _post_create(sentinel_token: str = "", so_token: str = ""):
            kwargs = {
                "json": payload,
                "headers": _build_create_headers(sentinel_token, so_token),
                "timeout": 30,
                "allow_redirects": False,
            }
            if impersonate:
                kwargs["impersonate"] = impersonate
            self._browser_pause()
            return self._session_post(request_url, **kwargs)
        ...
        try:
            ...
            if (
                ...
                if browser_token:
                    sentinel_token = str(browser_token)
                    so_token_str = getattr(browser_token, "so_token", None)
                else:
                    sentinel_token = build_sentinel_token(
                        self.session,
                        device_id,
                        ...
                        platform=self._fingerprint.platform if hasattr(self, '_fingerprint') and self._fingerprint else "Windows",
                        is_firefox=self.is_firefox,
                        viewport_width=self._fingerprint.viewport_width if hasattr(self, '_fingerprint') and self._fingerprint else 1920,
                        viewport_height=self._fingerprint.viewport_height if hasattr(self, '_fingerprint') and self._fingerprint else 1080,
                        logger=lambda msg: self._log(f"oauth_create_account: {msg}"),
                    )
                if not sentinel_token:
                    self._set_error("无法获取 sentinel token (oauth_create_account)")
                    return None

                r = _post_create(sentinel_token, so_token=so_token_str or "")
    ...
    def _fetch_consent_page_html(self, consent_url, user_agent, impersonate):
        ...
        try:
            ...
            if impersonate:
                kwargs["impersonate"] = impersonate
            self._browser_pause(0.12, 0.3)
            r = self._session_get(consent_url, **kwargs)
    ...
    def _exchange_code_for_tokens(self, code, code_verifier, user_agent, impersonate):
        ...
        try:
            ...
            self._browser_pause()
            r = self._session_post(url, **kwargs)
    ...
    def _send_phone_number(self, phone, device_id, user_agent, sec_ch_ua, impersonate):
        ...
        try:
            ...
            self._browser_pause(0.12, 0.25)
            resp = self._session_post(request_url, **kwargs)
        except Exception as e:
    ...
    def _dump_client_auth_session(self, referer=None):
        ...
        url = f"{self.oauth_issuer}/api/accounts/client_auth_session_dump"
        ...
        *,
        ...
        def _resend_email_otp() -> bool:
            ...
            try:
                ...
                if impersonate:
                    kwargs["impersonate"] = impersonate
                self._browser_pause()
                resp = self._session_get(request_url, **kwargs)
        ...
        try:
            otp_grace_seconds = int(
                self.config.get(
                    "chatgpt_oauth_otp_grace_seconds",
                    self.config.get("chatgpt_otp_grace_seconds", 30),
                )
                or 30
            )
        except Exception:
            otp_grace_seconds = 30
        otp_grace_seconds = max(0, min(otp_grace_seconds, 120))
        otp_deadline = time.time() + otp_wait_seconds
        otp_sent_at = _otp_sent_at_baseline - otp_grace_seconds
        self._log(
            f"OAuth OTP 等待窗口: total={otp_wait_seconds}s, poll_window={otp_poll_window}s, "
            f"grace={otp_grace_seconds}s, 每轮最多 5 次无响应后重发，最多 3 轮"
        )
        ...
        def validate_otp(code):
            ...
            try:
                ...
                if impersonate:
                    kwargs["impersonate"] = impersonate
                self._browser_pause(0.12, 0.25)
                resp_otp = self._session_post(request_url, **kwargs)
            except Exception as e:
                    ...
                    if no_new_count >= _max_no_new:
                        if resend_round < _max_resend_rounds:
                            ...
                            if _resend_email_otp():
                                otp_sent_at = time.time() - otp_grace_seconds
                            no_new_count = 0
                        else:
                            self._log(
